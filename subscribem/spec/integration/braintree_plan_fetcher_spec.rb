...
describe BraintreePlanFetcher do
  ...
  it "checks and updates plans" do
    Braintree::Plan.should_receive(:all).and_return([faux_plan])
    Subscribem::Plan.should_receive(:find_by_braintree_id).with(faux_plan.id).and_return(plan = double)
    plan.should_receive(:update_attributes).with({name: "Starter", price: "9.95"})
