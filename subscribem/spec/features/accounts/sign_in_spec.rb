...
feature "User sign in" do
  ...
  let!(:account) {FactoryGirl.create(:account)}  #_with_schema)}
  ...
  within_account_subdomain do
    ...
    scenario "attempts to sign in with ivalid email address and fails" do
      visit subscribem.root_url(subdomain: account.subdomain)
      expect(page.current_url).to eq(sign_in_url)
      expect(page).to have_content("Please sign in")
    ...
    scenario "user cannot sign in if not part of this subdomain" do
